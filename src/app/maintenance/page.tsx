...
export default function MaintenancePage() {
  ...
  const handleRetry = async () => {
    setIsRetrying(true);
    try {
      const response = await ContentsService.healthCheck();
      const responseKesiswaan = await ContentsService.healthCheckKesiswaan();
      if (
        response?.status.toLowerCase() === "ok" &&
        responseKesiswaan?.status.toLowerCase() === "ok"
      ) {
        router.push("/");
      }
    } catch (error) {
        ...
        <div className="max-w-2xl w-full">
          <div className="bg-white rounded-3xl shadow-lg p-8 md:p-12 border border-gray-200">
            ...
            <p className="text-center text-gray-500 text-sm mt-8">
              Jika masalah berlanjut, silakan hubungi pihak sekolah
            </p>
          </div>
        </div>
